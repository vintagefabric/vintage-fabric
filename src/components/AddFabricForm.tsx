...
/** Create a new fabric type (quality). */
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const res = await fetch("/api/admin/qualities", {
        ...
        body: JSON.stringify({ name, fabricType, width, composition, foil, heroImage }),
      });
  ...
  if (status === "done") {
    return (
      <div className="rounded-2xl border border-gold/40 bg-white p-8 text-center shadow-card">
        <h2 className="text-2xl">Fabric type created</h2>
    ...
    <form onSubmit={onSubmit} className="rounded-2xl border border-ink/10 bg-white p-6 shadow-card sm:p-8">
      <div className="grid gap-5 sm:grid-cols-2">
        <label className="block text-sm">
          ...
          <input name="name" required className={inputClass} placeholder="e.g. 60×60 Cambric Procian" />
        </label>
        <label className="block text-sm">
          <span className="font-medium text-ink">Fabric type *</span>
          <input name="fabricType" required className={inputClass} placeholder="e.g. 60×60 Cambric" />
        </label>
        <label className="block text-sm">
          <span className="font-medium text-ink">Width *</span>
          <input name="width" required className={inputClass} placeholder={'e.g. 58"'} />
        </label>
        <label className="block text-sm">
          <span className="font-medium text-ink">Composition *</span>
          <input name="composition" required className={inputClass} placeholder="e.g. 100% Cotton" />
        </label>
      </div>

      <label className="mt-5 flex items-center gap-2 text-sm">
        <input type="checkbox" name="foil" className="h-4 w-4 rounded border-ink/30 text-wine focus:ring-wine" />
      ...
      <button type="submit" disabled={status === "working"} className="btn-primary mt-6 w-full sm:w-auto">
        {status === "working" ? "Saving…" : "Create fabric type"}
      </button>
    </form>
  );
}
